...
try {
  const m = await import("../extra/order-notes.ts");
...
const cmd = new Command()
  .description("Manage Order Notes.")
  .action(() => {
    cmd.showHelp();
  })
  .command("list", "List Order Notes.\n\nhttps://docs.lana.dev/commerce/query/orderNotes")
  ...
  .action(async (opts) => {
    ...
    let req = request(ctx, "GET:order_notes.json")
  ...
  .command("get [...ids]", "Get one or multiple Order Notes.\n\nhttps://docs.lana.dev/commerce/query/orderNotes")
  .option("--order-id <string:string>", "Unique order identifier.", { required: true })
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
  ...
  .command("delete <...ids>", "Delete one or multiple Order Notes.\n\nhttps://docs.lana.dev/commerce/mutation/orderNotesDelete")
  .option("--order-id <string:string>", "Unique order identifier.", { required: true })
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "DELETE:order_notes.json")
    req = req.ids(ids)
    if (opts.orderId !== undefined) req = req.order_id(opts.orderId)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:order_notes.json");
    if (opts.orderId !== undefined) req = req.order_id(opts.orderId)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:order_notes.json").ids(ids);
    if (opts.orderId !== undefined) req = req.order_id(opts.orderId)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    req = req.data(assembleInputData(opts, true, {
      rawContent: "raw_content",
