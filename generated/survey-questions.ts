...
try {
  const m = await import("../extra/survey-questions.ts");
...
const cmd = new Command()
  ...
  .command("list", "List Survey Questions.\n\nhttps://docs.lana.dev/commerce/query/surveyQuestions")
  ...
  .action(async (opts) => {
    ...
    let req = request(ctx, "GET:survey_questions.json")
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    const resp = await req.sendUnwrap();
    printValues(resp, opts.format);
  })

  .command("get [...ids]", "Get one or multiple Survey Questions.\n\nhttps://docs.lana.dev/commerce/query/surveyQuestions")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
    value: formatParser("{id:v=>v.id,name:v=>v.name}"),
  ...
  .command("delete <...ids>", "Delete one or multiple Survey Questions.\n\nhttps://docs.lana.dev/commerce/mutation/surveyQuestionsDelete")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "DELETE:survey_questions.json")
    req = req.ids(ids)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    await req.sendUnwrap();
  })

  .command("create", "Create one or multiple Survey Questions.\n\nhttps://docs.lana.dev/commerce/mutation/surveyQuestionsCreate")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--kind <enum>", "(required) A kind of a survey question.")
  .option("--name <string>", "(required) The survey question itself.")
  .option("--answers <json>", "", { value: v => JSON.parse(v) })
  .option("--meta <string>", "Metadata of the option.")
  .option("--data <data>", "Input JSON data file or \"-\" for stdin")
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:survey_questions.json");
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .command("modify <ids...>", "Modify one or multiple Survey Questions.\n\nhttps://docs.lana.dev/commerce/mutation/surveyQuestionsModify")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--answers <json>", "", { value: v => JSON.parse(v) })
  .option("--kind <enum>", "A kind of a survey question.")
  .option("--meta <string>", "Metadata of the option.")
  .option("--name <string>", "The survey question itself.")
  .option("--data <data>", "Input JSON data file or \"-\" for stdin")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:survey_questions.json").ids(ids);
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    req = req.data(assembleInputData(opts, true, {
      answers: "answers",
      kind: "kind",
  ...
  .command("export [output]", "Export Survey Questions.\n\nGet info on available CSV columns using this command: `info-csv-format get --name surveyquestion`\n\nhttps://docs.lana.dev/commerce/mutation/surveyQuestionsExport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, output) => {
    ...
    const ctx = getRequestContext();
    let req = request(ctx, "POST:survey_questions/export.json");
    ...
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Exporting Survey Questions.");
  ...
  .command("import <input>", "Import Survey Questions.\n\nGet info on available CSV columns using this command: `info-csv-format get --name surveyquestion`\n\nhttps://docs.lana.dev/commerce/mutation/surveyQuestionsImport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, input) => {
    ...
    let req = request(ctx, "POST:survey_questions/import.json");
    req = req.shop_id(shopID)
    req = req.data({
      file_id: fileID,
      columns: opts.columns,
    ...
    const taskID = resp.task?.id;
    if (!taskID) throw new Error("task id is missing in response");
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Importing Survey Questions.");
