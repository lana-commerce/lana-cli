...
try {
  const m = await import("../extra/product-field-sets.ts");
...
const cmd = new Command()
  .description("Manage Product Field Sets.")
  .action(() => {
    cmd.showHelp();
  })
  .command("list", "List Product Field Sets.\n\nhttps://docs.lana.dev/commerce/query/productFieldSets")
  ...
  .action(async (opts) => {
    ...
    let req = request(ctx, "GET:product_field_sets.json")
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    const resp = await req.sendUnwrap();
    printValues(resp, opts.format);
  })

  .command("get [...ids]", "Get one or multiple Product Field Sets.\n\nhttps://docs.lana.dev/commerce/query/productFieldSets")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
    value: formatParser("{id:v=>v.id,name:v=>v.name}"),
  ...
  .command("delete <...ids>", "Delete one or multiple Product Field Sets.\n\nhttps://docs.lana.dev/commerce/mutation/productFieldSetsDelete")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "DELETE:product_field_sets.json")
    req = req.ids(ids)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    await req.sendUnwrap();
  })

  .command("create", "Create one or multiple Product Field Sets.\n\nhttps://docs.lana.dev/commerce/mutation/productFieldSetsCreate")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--fields <json>", "(required).", { value: v => JSON.parse(v) })
  .option("--name <string>", "(required) Name of the field set.")
  .option("--data <data>", "Input JSON data file or \"-\" for stdin")
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:product_field_sets.json");
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .command("modify <ids...>", "Modify one or multiple Product Field Sets.\n\nhttps://docs.lana.dev/commerce/mutation/productFieldSetsModify")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--fields <json>", "", { value: v => JSON.parse(v) })
  .option("--name <string>", "Name of the field set.")
  .option("--data <data>", "Input JSON data file or \"-\" for stdin")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:product_field_sets.json").ids(ids);
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    req = req.data(assembleInputData(opts, true, {
      fields: "fields",
      name: "name",
  ...
  .command("export [output]", "Export Product Field Sets.\n\nGet info on available CSV columns using this command: `info-csv-format get --name productfieldset`\n\nhttps://docs.lana.dev/commerce/mutation/productFieldSetsExport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, output) => {
    ...
    const ctx = getRequestContext();
    let req = request(ctx, "POST:product_field_sets/export.json");
    ...
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Exporting Product Field Sets.");
  ...
  .command("import <input>", "Import Product Field Sets.\n\nGet info on available CSV columns using this command: `info-csv-format get --name productfieldset`\n\nhttps://docs.lana.dev/commerce/mutation/productFieldSetsImport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, input) => {
    ...
    let req = request(ctx, "POST:product_field_sets/import.json");
    req = req.shop_id(shopID)
    req = req.data({
      file_id: fileID,
      columns: opts.columns,
    ...
    const taskID = resp.task?.id;
    if (!taskID) throw new Error("task id is missing in response");
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Importing Product Field Sets.");
