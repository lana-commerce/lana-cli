...
try {
  const m = await import("../extra/option-sets.ts");
...
const cmd = new Command()
  .description("Manage Option Sets.")
  .action(() => {
    cmd.showHelp();
  })
  .command("list", "List Option Sets.\n\nhttps://docs.lana.dev/commerce/query/optionSets")
  ...
  .action(async (opts) => {
    ...
    let req = request(ctx, "GET:option_sets.json")
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    const resp = await req.sendUnwrap();
    printValues(resp, opts.format);
  })

  .command("get [...ids]", "Get one or multiple Option Sets.\n\nhttps://docs.lana.dev/commerce/query/optionSets")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
    value: formatParser("{id:v=>v.id,name:v=>v.name}"),
  ...
  .command("delete <...ids>", "Delete one or multiple Option Sets.\n\nhttps://docs.lana.dev/commerce/mutation/optionSetsDelete")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "DELETE:option_sets.json")
    req = req.ids(ids)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    await req.sendUnwrap();
  })

  .command("create", "Create one or multiple Option Sets.\n\nhttps://docs.lana.dev/commerce/mutation/optionSetsCreate")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--name <string>", "(required) Name of the option set.")
  .option("--options <json>", "(required) Option IDs of options which belong to a set.", { value: v => JSON.parse(v) })
  .option("--data <data>", "Input JSON data file or \"-\" for stdin")
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:option_sets.json");
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .command("modify <ids...>", "Modify one or multiple Option Sets.\n\nhttps://docs.lana.dev/commerce/mutation/optionSetsModify")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--name <string>", "Name of the option set.")
  .option("--options <json>", "Option IDs of options which belong to a set.", { value: v => JSON.parse(v) })
  .option("--data <data>", "Input JSON data file or \"-\" for stdin")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:option_sets.json").ids(ids);
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    req = req.data(assembleInputData(opts, true, {
      name: "name",
      options: "options",
  ...
  .command("export [output]", "Export Option Sets.\n\nGet info on available CSV columns using this command: `info-csv-format get --name optionset`\n\nhttps://docs.lana.dev/commerce/mutation/optionSetsExport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, output) => {
    ...
    const ctx = getRequestContext();
    let req = request(ctx, "POST:option_sets/export.json");
    ...
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Exporting Option Sets.");
  ...
  .command("import <input>", "Import Option Sets.\n\nGet info on available CSV columns using this command: `info-csv-format get --name optionset`\n\nhttps://docs.lana.dev/commerce/mutation/optionSetsImport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, input) => {
    ...
    let req = request(ctx, "POST:option_sets/import.json");
    req = req.shop_id(shopID)
    req = req.data({
      file_id: fileID,
      columns: opts.columns,
    ...
    const taskID = resp.task?.id;
    if (!taskID) throw new Error("task id is missing in response");
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Importing Option Sets.");
