...
const purchaseOrderCategory = new EnumType(["any", "closed", "draft", "not_received", "open", "partially_received", "past_due", "received"]);
const purchaseOrdersPageSortBy = new EnumType(["created_at"]);
const searchPurchaseOrdersSortBy = new EnumType(["created_at", "expected_at", "from_supplier", "number", "status", "to_inventory_location", "updated_at"]);

let addExtraCommands = (cmd: Command) => cmd;
try {
  const m = await import("../extra/purchase-orders.ts");
...
const cmd = new Command()
  ...
  .command("list", "List Purchase Orders.\n\nhttps://docs.lana.dev/commerce/query/purchaseOrdersPage")
  .type("purchaseOrderCategory", purchaseOrderCategory)
  .type("purchaseOrdersPageSortBy", purchaseOrdersPageSortBy)
  ...
  .option("--sort-by <enum:purchaseOrdersPageSortBy>", "Whether to sort output in some specific way.")
  .option("--sort-desc", "Whether to use descending sort order.")
  .option("--supplier-id <string:string>", "Filter output by supplier.")
  .option("--type <enum:purchaseOrderCategory>", "Filter purchase orders output by a purchase order type.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
    value: formatParser("{id:v=>v.id,number:v=>v.number,currency:v=>v.currency,status:v=>v.status,created:v=>new Date(v.created_at).toLocaleString()}", "{id:v=>v.id,number:v=>v.number,currency:v=>v.currency,status:v=>v.status,created_at:v=>v.created_at}"),
  })
  .option("--stream", "Stream paginated output instead of returning a single page.")
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "GET:purchase_orders/page.json")
    if (opts.stream) {
      ...
      if (opts.supplierId !== undefined) req = req.supplier_id(opts.supplierId)
  ...
  .command("get [...ids]", "Get one or multiple Purchase Orders.\n\nhttps://docs.lana.dev/commerce/query/purchaseOrders")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
    value: formatParser("{id:v=>v.id,number:v=>v.number,currency:v=>v.currency,status:v=>v.status,created:v=>new Date(v.created_at).toLocaleString()}", "{id:v=>v.id,number:v=>v.number,currency:v=>v.currency,status:v=>v.status,created_at:v=>v.created_at}"),
  ...
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "GET:purchase_orders.json")
    req = req.ids(ids)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    const resp = await req.sendUnwrap();
  ...
  .command("delete <...ids>", "Delete one or multiple Purchase Orders.\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersDelete")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "DELETE:purchase_orders.json")
    req = req.ids(ids)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    await req.sendUnwrap();
  })

  .command("create", "Create one or multiple Purchase Orders.\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersCreate")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--bill-to <json>", "Bill to address.", { value: v => JSON.parse(v) })
  .option("--currency <string>", "Currency code.")
  .option("--date <datetime>", "Order date.")
  .option("--draft <boolean:boolean>", "Whether purchase order is in a draft state or not.")
  .option("--expected-at <datetime>", "Date and time when purchase order shipment is expected.")
  .option("--from-supplier-id <string>", "Unique supplier id (source).")
  .option("--line-items <json>", "", { value: v => JSON.parse(v) })
  .option("--reference <string>", "Arbitrary reference string.")
  .option("--send-email <boolean:boolean>", "Whether to send this PO to supplier via email or not (when submitted).")
  .option("--shipping <number:number>", "Shipping price.")
  .option("--supplier-note <string>", "Note that will be visible to supplier (as part of the email).")
  .option("--tax-inclusive-prices <boolean:boolean>", "Whether prices include the tax or not.")
  .option("--to-inventory-location-id <string>", "Unique inventory location id (destination).")
  .option("--data <data>", "Input JSON data file or \"-\" for stdin")
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:purchase_orders.json");
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .command("modify <ids...>", "Modify one or multiple Purchase Orders.\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersModify")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--bill-to <json>", "Bill to address.", { value: v => JSON.parse(v) })
  .option("--currency <string>", "Currency code.")
  .option("--date <datetime>", "Order date.")
  ...
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:purchase_orders.json").ids(ids);
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    req = req.data(assembleInputData(opts, true, {
      billTo: "bill_to",
      currency: "currency",
  ...
  .command("search", "Search Purchase Orders.\n\nhttps://docs.lana.dev/commerce/query/searchPurchaseOrders")
  .type("searchPurchaseOrdersSortBy", searchPurchaseOrdersSortBy)
  .option("--limit <number:number>", "Return up to N entries (pagination).")
  .option("--offset <number:number>", "Skip N entries (pagination).")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--sort-by <enum:searchPurchaseOrdersSortBy>", "Whether to sort output in some specific way.")
  .option("--sort-desc", "Whether to use descending sort order.")
  ...
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:search/purchase_orders.json");
  ...
  .command("suggest <...query>", "Suggest Purchase Orders.\n\nhttps://docs.lana.dev/commerce/query/suggestPurchaseOrders")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
    value: formatParser("{id:v=>v.id,number:v=>v.number,currency:v=>v.currency,status:v=>v.status,created:v=>new Date(v.created_at).toLocaleString()}", "{id:v=>v.id,number:v=>v.number,currency:v=>v.currency,status:v=>v.status,created_at:v=>v.created_at}"),
  ...
  .action(async (opts, ...query) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:suggest/purchase_orders.json");
    req = req.expand({ items: true });
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .command("receive", "Receive Purchase Order Items.\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersReceive")
  ...
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:purchase_orders/receive.json");
  ...
  .command("reject", "Reject Purchase Order Items.\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersReject")
  .option("--purchase-order-id <string:string>", "Unique purchase order identifier.", { required: true })
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--line-items <json>", "", { value: v => JSON.parse(v) })
  ...
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:purchase_orders/reject.json");
    if (opts.purchaseOrderId !== undefined) req = req.purchase_order_id(opts.purchaseOrderId)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    req = req.data(assembleInputData(opts, false, {
      lineItems: "line_items",
  ...
  .command("open", "Open (unarchive) Purchase Order.\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersOpen")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:purchase_orders/open.json");
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    await req.sendUnwrap();
  })

  .command("close", "Close (archive) Purchase Order.\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersClose")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:purchase_orders/close.json");
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    await req.sendUnwrap();
  })

  .command("log", "Get Purchase Order log.\n\nhttps://docs.lana.dev/commerce/query/purchaseOrdersLog")
  .option("--purchase-order-id <string:string>", "Unique purchase order identifier.", { required: true })
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .option("--format <format>", "Format the output in a specific way.", {
    default: "table",
  ...
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "GET:purchase_orders/log.json");
    if (opts.purchaseOrderId !== undefined) req = req.purchase_order_id(opts.purchaseOrderId)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    const resp = await req.sendUnwrap();
    printValues(resp.items, opts.format);
  ...
  .command("export [output]", "Export Purchase Orders.\n\nGet info on available CSV columns using this command: `info-csv-format get --name purchaseorder`\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersExport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, output) => {
    ...
    const ctx = getRequestContext();
    let req = request(ctx, "POST:purchase_orders/export.json");
    ...
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Exporting Purchase Orders.");
  ...
  .command("import <input>", "Import Purchase Orders.\n\nGet info on available CSV columns using this command: `info-csv-format get --name purchaseorder`\n\nhttps://docs.lana.dev/commerce/mutation/purchaseOrdersImport")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, input) => {
    ...
    let req = request(ctx, "POST:purchase_orders/import.json");
    req = req.shop_id(shopID)
    req = req.data({
      file_id: fileID,
      columns: opts.columns,
    ...
    const taskID = resp.task?.id;
    if (!taskID) throw new Error("task id is missing in response");
    const t = await waitForTaskWithProgressBar(ctx, shopID, taskID, "Importing Purchase Orders.");
