...
const cmd = new Command()
  ...
  .command("list", "List Payment Methods.\n\nhttps://docs.lana.dev/commerce/query/paymentMethods")
  ...
  .action(async (opts) => {
    ...
    let req = request(ctx, "GET:payment_methods.json")
  ...
  .command("delete <...ids>", "Delete one or multiple Payment Methods.\n\nhttps://docs.lana.dev/commerce/mutation/paymentMethodsDelete")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "DELETE:payment_methods.json")
    req = req.ids(ids)
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    await req.sendUnwrap();
  })

  .command("create", "Create one or multiple Payment Methods.\n\nhttps://docs.lana.dev/commerce/mutation/paymentMethodsCreate")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:payment_methods.json");
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
  ...
  .command("modify <ids...>", "Modify one or multiple Payment Methods.\n\nhttps://docs.lana.dev/commerce/mutation/paymentMethodsModify")
  .option("--shop-id <string:string>", "Unique shop identifier.")
  ...
  .action(async (opts, ...ids) => {
    const ctx = getRequestContext();
    await updateAllCacheEntries(ctx, opts.shopId || getConfigValue("shop_id"));
    let req = request(ctx, "POST:payment_methods.json").ids(ids);
    req = req.shop_id(opts.shopId || getConfigValue("shop_id"))
    req = req.data(assembleInputData(opts, true, {
